<footer class="site-footer">
  <div class="footer-inner">
    <!-- Rechtsgebieden -->
    <div class="footer-section">
      <h3>RECHTSGEBIEDEN</h3>
      <ul class="footer-links-list">
        <li><a href="info.php?page=arbeid">Arbeidsrecht</a></li>
        <li><a href="info.php?page=gezinshereniging">Gezinshereniging</a></li>
        <li><a href="info.php?page=schengenvisum">Schengenvisum</a></li>
        <li><a href="info.php?page=studeren-in-nederland">Studeren in Nederland</a></li>
        <li><a href="info.php?page=verblijfsvergunning-onbepaalde-tijd-aanvragen">Verblijfsvergunning onbepaalde tijd</a></li>
      </ul>
    </div>

    <!-- Contactgegevens -->
    <div class="footer-section">
      <h3>Contactgegevens</h3>
      <div class="footer-contact">
        <p class="footer-phone"><a href="tel:+31297548241">+31297548241</a></p>
        <p class="footer-phone"><a href="tel:+31648154534">+316 481 54 534</a></p>
        <p class="footer-email"><a href="mailto:info@linkhofJuristen.nl">info@nevanexis.nl</a></p>
        <div class="footer-company-info">
          <p><strong>Kvk-nummer:</strong> 78407079</p>
          <p><strong>Btw-nummer:</strong> NL003331049B34</p>
          <p><strong>Rekeningnummer:</strong> NL93INGB0009304720</p>
        </div>
      </div>
    </div>

    <!-- Kaart -->
    <div class="footer-section footer-map">
      <div id="footer-map" style="height: 300px; width: 100%; border-radius: 8px; overflow: hidden;"></div>
    </div>
  </div>

  <div class="footer-bottom">
    <p>© 2023 - 2026 nevanexis</p>
    <a href="#" class="footer-disclaimer">Schermafbeelding</a>
    <p>Powered by <a href="https://copilot.com">Copilot</a></p>
  </div>
</footer>

<a href="https://wa.me/31297548241" class="whatsapp-float" target="_blank" rel="noopener noreferrer" title="Stuur ons een WhatsApp bericht naar +31 297 548 241">
  <svg viewBox="0 0 24 24" width="24" height="24" aria-hidden="true" style="width:60%;height:60%;">
    <path fill="#ffffff" d="M20.52 3.48A11.95 11.95 0 0012 0C5.37 0 .08 5.29.08 11.92c0 2.1.58 4.15 1.68 5.94L0 24l6.41-1.64A11.92 11.92 0 0012 24c6.63 0 12-5.37 12-12 0-3.2-1.25-6.2-3.48-8.52zM12 21.5c-1.86 0-3.66-.5-5.23-1.44l-.37-.22-3.8 1.01 1.02-3.5-.24-.36A9.49 9.49 0 012.5 11.92 9.5 9.5 0 1112 21.5z"/>
    <path fill="#ffffff" d="M17.05 14.02c-.28-.14-1.65-.82-1.9-.91-.25-.09-.43-.13-.62.14-.18.27-.71.9-.87 1.09-.16.19-.32.22-.6.08-.28-.14-1.18-.43-2.24-1.39-.82-.73-1.32-1.6-1.48-1.87-.16-.27-.02-.43.12-.56.12-.11.28-.28.42-.42.14-.14.18-.24.28-.4.09-.16.04-.3.02-.43-.02-.13-.6-1.37-.82-1.88-.22-.51-.45-.44-.62-.45-.16-.01-.33-.01-.5-.01-.18 0-.45.02-.69.32-.24.3-.89.96-.89 2.34 0 1.38 1.1 3.04 1.26 3.31.16.27 2.18 3.33 5.6 4.66 3.42 1.33 3.42.99 3.82.92.4-.07 1.27-.52 1.45-1 .18-.48.18-.9.12-1 .06-.08-.64-.5-.92-.64z"/>
  </svg>
</a>

<script src="js/main.js" defer></script>

</body>
</html>
